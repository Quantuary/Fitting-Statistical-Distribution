...
# fit = DS.fit_distribution(data["CLAIM_INCURRED"], 'Severity')
# best_fit_params = fit.compare(dist=['gamma','expon'])
# 
#  
# mean, var, skew, kurt = st.gamma.stats(a=best_fit_params[0],
...
# =============================================================================
# # Plotting Best fit PDF
# def plot_pdf(fitted_object, best_fit_name, best_fit_params):
#     best_dist = getattr(st, best_fit_name)
#     pdf = fitted_object.make_pdf(best_dist, best_fit_params)
#              
#     #Display
#     plt.figure(figsize=(16,8))
#     ax = pdf.plot(lw=2, label='PDF', legend=True)
#     fitted_object.data.plot(kind='hist', bins=250, density=True, alpha=0.5, label='Data', legend=True, ax=ax)
#     #         
#     param_names = (best_dist.shapes + ', loc, scale').split(', ') if best_dist.shapes else ['loc', 'scale']
#     param_str = ', '.join(['{}={:0.2f}'.format(k,v) for k,v in zip(param_names, best_fit_params)])
#     dist_str = '{}({})'.format(best_fit_name, param_str)
#     x=best_dist.stats(*best_fit_params,moments='m')
#     plt.axvline(x,linestyle="--",color='r')
#     plt.text(x+10, 0.00008, "$\mu=$", fontsize=12, color="r")
#     plt.text(x+100, 0.00008, '${:,.0f}'.format(x), fontsize=12, color="r")
#     
#     dataYLim = ax.get_ylim()
#     ax.set_ylim(dataYLim)
#     ax.set_title('%s Distribution\n'%(fitted_object.name) + dist_str )
#     ax.xaxis.set_major_formatter(matplotlib.ticker.StrMethodFormatter('${x:,.0f}'))
#     ax.set_xlabel('$AUD')
#     ax.set_ylabel('Probability')
#     
# plot_pdf(fit, 'gamma', best_fit_params)
# 
# =============================================================================